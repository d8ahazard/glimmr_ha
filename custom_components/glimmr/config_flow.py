...
from .const import CONF_KEEP_MASTER_LIGHT, DEFAULT_KEEP_MASTER_LIGHT, DOMAIN, LOGGER
...
class GlimmrFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_zeroconf(
        ...
        # Hostname is format: glimmr-livingroom.local.
    ...
    async def _handle_config_flow(
        ...
        if user_input.get(CONF_MAC) is None or not prepare:
            ...
            LOGGER.debug("Creating glimmr uising " + user_input[CONF_HOST])
            glimmr = Glimmr(user_input[CONF_HOST], session=session)
            glimmr.set_logger(LOGGER)
            try:
            ...
            user_input[CONF_MAC] = device.device_id
